...
contract AssetToken is ERC20 {
    ...
    //////////////////////////////////////////////////////////////*/
    constructor(
        address thunderLoan, // the actual thunderLoan contract address
        IERC20 underlying, // token being deposited for getting a flash loan
        string memory assetName,
        string memory assetSymbol
        // q are tokens stored inside the AssetToken contract instead of the ThunderLoan contract ???
    )
        ERC20(assetName, assetSymbol) // initialize the asset token with a name and symbol
    ...
    // e only the ThunderLoan contract can mint new tokens ???
    ...
    // e This function is responsible for updating the exchange rate of the asset token -> underlying token
    // for example if the exchange rate is 1.5, it means 1 asset token is worth 1.5 underlying tokens
    // q Why the exchange rate should always go up and never down ??? (INVARIANT)
